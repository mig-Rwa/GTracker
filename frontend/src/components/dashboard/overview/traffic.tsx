...
export interface WorkoutTypeDistributionProps {
  chartSeries: number[];
  labels: string[];
  sx?: SxProps;
}

export function WorkoutTypeDistribution({ chartSeries, labels, sx }: WorkoutTypeDistributionProps): React.JSX.Element {
  const chartOptions = useChartOptions(labels);

  return (
    <Card sx={sx}>
      <CardHeader title="Workout Type Distribution" />
      <CardContent>
        <Stack spacing={2}>
